...
class spam_model extends CI_Model {
	...
	function get_by_sender_and_receiver($sender,$receiver){
		$select = '*';
		$array_where = array('sender' => $sender,'receiver'=>$receiver);
		$array_like = array();
		$order_by = array();
		return $this -> get($select, $array_where, $array_like, 0, 1, array('id'=>'DESC'));
	}
	...
	function insert($data_array) {
		...
		$this -> db -> insert('spams', $data_array);
		return $this -> db -> insert_id();
	}

	public function remove($arr_where) {
		$this -> db -> where($arr_where);
		$this -> db -> delete('spams');
	...
	function update($data_array, $array_where) {
		$this -> db -> where($array_where);
		$this -> db -> update('spams', $data_array);
	}
}
?>