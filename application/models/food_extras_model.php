<?php
/**
Location:application/food_extras.php
*/
class food_extras_model extends CI_Model {
  ...
  function get($select = "price,name", $array_where = false, $array_like = false, $first = false, $offset = false, $order_by = false) {
    ...
    $this->db->join('extras','food_extras.extra_id = extras.id','left');
    $query = $this -> db -> get();
    if ($query -> num_rows() > 0) {
  ...
  function insert($data_array) {
    ...
    $this -> db -> insert('food_extras', $data_array);
    return $this -> db -> insert_id();
  }

  public function remove($arr_where) {
    $this -> db -> where($arr_where);
    $this -> db -> delete('food_extras');
  ...
  function update($data_array, $array_where) {
    $this -> db -> where($array_where);
    $this -> db -> update('food_extras', $data_array);
  }
}
?>
